...
if [[ $contest_name == "warmup" ]]; then
  cat $SCRIPT_DIR/cover.tex | sed -e 's/30th/29th/g' -e 's/Contest Session/Practice Session/g' -e "s/REVNUM/$(git rev-parse --short HEAD)/g" > $RELEASE/cover.tex
fi
...
if command -v pdftk &> /dev/null; then
  pdftk $RELEASE/cover.pdf $RELEASE/statement.pdf cat output $RELEASE/${contest_name}.pdf && rm -rf $RELEASE/{cover,statement}.pdf
fi
