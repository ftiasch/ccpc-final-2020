...
int &at(int i, int carry, int sum) { return dp[i & 1][carry + 1][sum + XM]; }

int &at(int i, int sum) { return small_dp[i & 1][sum + VM]; }
...
int main() {
  ...
  while (scanf("%s", s) == 1) {
    ...
    for (int i = 5; i >= 1; --i) {
      int o = s[i - 1] - '0';
      for (int sum = -VM; sum <= VM; ++sum) {
        int &w = at(i, sum);
        if (w == -1)
          continue;
        for (int x = -x_bound; x <= x_bound; ++x) {
          int new_sum = sum + x * p10 - x - o * p10;
          if (-VM <= new_sum && new_sum <= VM) {
            update(at(i - 1, new_sum), w + std::abs(x) * (i - 1));
          }
        }
        w = -1;
      }
