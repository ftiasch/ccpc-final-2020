...
int& at(int i, int carry, int sum) {
  return dp[i & 1][carry + 5][sum + 25000];
}

int& at(int i, int sum) {
  return small_dp[i & 1][sum + M];
}
  ...
  while (scanf("%s", s) == 1) {
    ...
    for (int i = 5; i >= 1; --i) {
      ...
      for (int sum = -M; sum <= M; ++sum) {
        ...
        if (w == -1) continue;
        for (int x = -x_bound; x <= x_bound; ++x) {
          update(at(i - 1, sum + x * p10 - x - o * p10), w + std::abs(x) * (i - 1));
        }
      }
      p10 /= 10;
    }
